...
class EvaluationRunHook(tf.train.SessionRunHook):
    ...
    def _run_eval(self):
        ...
        coord = tf.train.Coordinator(
            clean_stop_exception_types=(tf.errors.CancelledError,
...
def run(target, is_chief, train_steps, eval_steps, job_dir, train_files,
        eval_files, train_batch_size, eval_batch_size, learning_rate,
        eval_frequency, first_layer_size, num_layers, scale_factor,
        eval_num_epochs, num_epochs):
    ...
    with tf.Graph().as_default():
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            with coord.stop_on_exception():
                while (train_steps is None or
                       step < train_steps) and not coord.should_stop():
        ...
        if is_chief:
            build_and_run_exports(latest_checkpoint, job_dir, 'CSV',
                                  model.csv_serving_input_fn, hidden_units)
            build_and_run_exports(latest_checkpoint, job_dir, 'JSON',
                                  model.json_serving_input_fn, hidden_units)
            build_and_run_exports(latest_checkpoint, job_dir, 'EXAMPLE',
                                  model.example_serving_input_fn, hidden_units)
